...
function showInterpResult(resId: string, bdId: string, label: string, targetRow: number, rowLabel: string, result: ReturnType<typeof interpolateBetweenRows>): void {
    ...
    if (exact) {
        renderKaTeXInto(bdEl, \`\\\\text{Valor exacto en tabla para } \${rowLabel} = \${targetRow}\`, true);
    } else {
        const tableHtml = \`
            <div class="mt-2 pt-2 border-t border-slate-200 dark:border-slate-600 w-full text-left">
                <p class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Desglose Regla de Tres Mútiple</p>
                <div class="overflow-x-auto w-full mb-4">
                    <table class="breakdown-table w-full text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 text-center">
                        <thead class="bg-slate-100 dark:bg-slate-700">
                            <tr><th>Punto</th><th class="whitespace-nowrap">$\\text{\${rowLabel}}$</th><th class="whitespace-nowrap">$\\text{Valor Crítico (v)}$</th></tr>
                        </thead>
                        <tbody>
                            <tr class="border-b border-slate-100 dark:border-slate-700">
                                <td class="font-semibold text-slate-500">Ant. $(x_1, y_1)$</td>
                            ...
                            <tr>
                                <td class="font-semibold text-slate-500">Sig. $(x_2, y_2)$</td>
        ...
        renderKaTeXInto(formulaEl, \`\\\\begin{aligned} y &= y_1 + \\\\dfrac{x - x_1}{x_2 - x_1} \\\\cdot (y_2 - y_1) \\\\\\\\ \${label} &= \${lo.v.toFixed(4)} + \\\\dfrac{\${targetRow} - \${lo.num}}{\${hi.num} - \${lo.num}} \\\\cdot (\${hi.v.toFixed(4)} - \${lo.v.toFixed(4)}) \\\\\\\\ &\\\\approx \${val.toFixed(4)} \\\\end{aligned}\`, true);
    }
}
`;
...
function showInvInterpResult(resId: string, bdId: string, label: string, targetVal: number, result: ReturnType<typeof interpolateInverseRow>): void {
    ...
    if (exact) {
        ...
        const tableHtml = \`
            <div class="mt-2 pt-2 border-t border-slate-200 dark:border-slate-600 w-full text-left">
                <p class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2 text-left">Desglose Regla de Tres (\${approxGl})</p>
                <div class="overflow-x-auto w-full mb-4">
                    <table class="breakdown-table w-full text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 text-center">
                        <thead class="bg-slate-100 dark:bg-slate-700">
                            <tr><th>Punto</th><th class="whitespace-nowrap">Valor Extremo $(x)$</th><th class="whitespace-nowrap">Probabilidad $\\text{\${label}} \\; (y)$</th></tr>
                        </thead>
                        <tbody>
                            <tr class="border-b border-slate-100 dark:border-slate-700">
                                <td class="font-semibold text-slate-500">Límite A $(x_1, y_1)$</td>
                            ...
                            <tr>
                                <td class="font-semibold text-slate-500">Límite B $(x_2, y_2)$</td>
        ...
        bdEl.innerHTML = tableHtml;
        const formulaEl = document.getElementById(bdId + '_formula')!;
        renderKaTeXInto(formulaEl, \`\\\\begin{aligned} y &= y_1 + \\\\dfrac{x - x_1}{x_2 - x_1} \\\\cdot (y_2 - y_1) \\\\\\\\ \${label} &= \${lo.a.toPrecision(4)} + \\\\dfrac{\${targetVal.toFixed(4)} - \${lo.v.toFixed(4)}}{\${hi.v.toFixed(4)} - \${lo.v.toFixed(4)}} \\\\cdot (\${hi.a.toPrecision(4)} - \${lo.a.toPrecision(4)}) \\\\\\\\ &\\\\approx \${val.toPrecision(4)} \\\\end{aligned}\`, true);
    }
}
`;
...
console.log("Main ts re-factor complete");
